    ...
    public class TestDiscoverer : ITestDiscoverer
    {
        ...
        internal static IEnumerable<TestCase> GetTestCases(string source, IMessageLogger logger)
        {
            ...
            foreach (var type in assembly.GetTypes())
            {
                ...
                var attributeFound = new SyncedTestAttribute(string.Empty);

                foreach (var a in type.GetCustomAttributes(syncedTestAttributeType, false))
                {
                    if(a is SyncedTestAttribute successfulCast)
                    {
                        typeFound = true;
                        attributeFound = successfulCast;
                        break;
                    }
                }
                ...
                var featureFileName = attributeFound.FeatureFileName;
